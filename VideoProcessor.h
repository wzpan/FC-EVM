...
public:
    ...
    // extractXTSlice
    cv::Mat extractXTSlice(int y);

    // extractYTSlice
    cv::Mat extractYTSlice(int x);
...
private:    
    ...
    // hue historgram of ROI
    cv::MatND colorhist;
    // mean-shift object finder
    ObjectFinder finder;
    // ROI image
    cv::Mat imageROI;
    ...
    bool createTemp(Size frameSize);
    ...
    // blending
    cv::Mat LaplacianBlend(const cv::Mat& l, const cv::Mat& r, const cv::Mat_<float>& m);
